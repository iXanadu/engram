...
adversarially reviewed by projalpha-grok). Every test here measures a WORLD
...
the joint work with Projalpha (G2). Cursor is a loud SKIP per G1
...
# surface). To run it: spawn a session through Projalpha, open the picker,
...
# in huddle DfNRCl6x, 2026-08-20, shape by projalpha-claude-3).
#
...
def test_arrival_channel_mail_after_restart_reaches_a_reused_name(
    ...
    (read_by ['engram-claude-3@hosta', 'engram-claude-2@hosta'] on a

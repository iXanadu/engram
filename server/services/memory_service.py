    ...
    async with pool.acquire() as conn:
        ...
        async with (
            ...
            if if_match == "":
                ...
                # and if_match_applied=true. Projalpha lost real content to
...
async def partition_siblings(
    ...
    "stored". Measured 2026-08-10 (projdelta): all three verbs reported a
...
#   scope     = 'inbox'
#   user_id   = <to address>   — e.g. 'engram', 'machine:hosta'
...
async def inbox_list(
    ...
    # messages invisible; caught by projalpha 2026-07-19). Display callers get
...
# by Projalpha: this is not a settling window a session passes through, it is
...
async def presence_update(
    ...
    async with pool.acquire() as conn:
        ...
        metadata = {
            ...
            # a dead hostb codex session's host survived on beats from a
...
async def presence_watcher_beat(identity: str, project: str) -> bool:
    ...
    * It must not INSERT. No presence row means the session has never
    ...
    async with pool.acquire() as conn:
        ...
        #
        # ⚠️ HOW FAR THIS ACTUALLY GETS — corrected 2026-08-01 after Projalpha
...
def _watcher_state(md: dict, now: datetime) -> tuple[bool | None, datetime | None]:
    ...
    Follows the same discipline Projalpha applies to its process-ancestry
...
async def unanswered_asks(min_age_hours: float = 4.0, limit: int = 100) -> list[dict]:
    ...
    from here. The spawner (Projalpha) knows, because it performs the turns.

...
# renewer, by decision (Projalpha's argument, adopted 2026-08-01).
#
...
def _meta(session_key: str, session_nonce: str | None, provider: str,
    ...
    return {
        ...
        # so the register can serve "wanted projalpha-app-grok, got grok-6"
...
async def seat_claim(
    ...
    never told. Registering was Projalpha's call and the right one: a silent
    ...
    async with pool.acquire() as conn:
        ...
        if held_rows:
            ...
            # to finish dying — Projalpha's grok path kills a tmux session and
            ...
            if door_closed:
                ...
                if (runtime_seat and preferred and preferred != seat
                    ...
                    # 2026-08-18: projalpha-app-grok's moved row lost it).
...
# live in the orchestration layer (Projalpha), never in an engram address.
...
async def seat_list(
    ...
    for r in rows:
        ...
        out.append({
            ...
            # SEATS-1 (requested by Projalpha 2026-07-28). Unlike is_live/
...
def _parse_tree_node(addr: str, roots: set[str]) -> tuple[str, str, str] | None:
    ...
    Longest root wins, so 'projalpha-app-grok' parses under
    'projalpha-app' when that root exists, not under 'projalpha'.
    """
    bare = addr.split("@", 1)[0]

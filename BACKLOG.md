...
- **ADDR-2** A send to an address nobody holds succeeds silently. Measured
  ...
  **ADDR-3, folded in here because it shares the code path — the register
  ...
  which is what let a human form `admin@hostb` from `project = admin` in the
  ...
  a SEAT. Did you mean `maintenance-claude-hostb`?"* — a typo report rather
...
- **DEPLOY-3** The spokes' watcher cannot report itself, so the roster's
  watcher field is a false negative fleet-wide. hostb, hostc and hostd
  ...
  ⚠️ **Blast radius is a peer's unbuilt feature:** Projalpha asked

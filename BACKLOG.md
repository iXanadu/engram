...
- **RELAY-1** Authorship of a relayed message lives in BODY PROSE, so a peer
  can wear the owner's stamp. Proposed by Projalpha 2026-08-07 (their pin,
...
- **SEATS-1** Serve `watcher_last_seen` (and `watcher_alive`) per seat on
  ...
  OTHER boxes has to fall back to a coarser signal. Requested by Projalpha
...
- **BEAT-1** The bridge has no background beat, so "last spoke" measures
  ...
  **dissolve SEAT-15 rather than decide it** — the first beat claims the
  ...
  interrupt. Same survivorship problem Projalpha raised against their own
...
- **SEAT-16** *(RAISED 2026-08-10 — a consumer now depends on this.)* The
  ...
  for several of nine ordinals on one project — diagnosed by Projalpha, who
...
- **DEADMAN-1** Cross-box dead-man probe for hosta — ON HOLD by the owner

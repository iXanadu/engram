...
- **PROV-CODEX-1** Codex sessions cannot send/read inbox mail while Cursor
  ...
  hosta (memory_get/resolve/store were unlisted; 11 tools added,
  destructive verbs deliberately left gated). Fleet parity DONE same
  evening on the owner's word: hostb carried the identical six-tool list,
  same 11-tool fix applied and TOML-verified (17 listed); hostc and
  hostd have no codex config (nothing to fix). Note: hostb's list

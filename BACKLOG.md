...
- **ID-1** An unconfigured session silently becomes `admin` for ADDRESSING,
  ...
  **Not just scratch dirs — production ones.** Rung 2 is a `/projects/`
  ...
  cfg: Projalpha, FleetBackup, Projzeta, engram. These do NOT and would
  ...
  half is Projalpha's (warn when a target would resolve to `admin`,
...
- **BRIDGE-1** (hardening, not urgent) The bridge's seat-claim path fails
  ...
  **Not observed firing.** Raised 2026-07-26 as the explanation for a peer
  ...
  with Projalpha's ledger where the numbers already mean other things.

- **SEAT-6** Grok seat integration is incomplete (Projalpha-owned, engram
...
- **SEAT-4** Roster lifecycle (registry Phase 2). Presence rows are never
  ...
  *(An earlier revision of this entry claimed ~41 minutes and "ends only when
  ...
  exactly as intended. Projalpha confirmed nothing had changed on seat
...
- **MSG-6** **Cross-machine delivery is UNTESTED.** Clients on other boxes
  ...
  boundary — every test to date has been hosta→hosta. A single
  ...
  is what Projalpha's HUD-4 migration target needed.)
...
- **MEM-2** Key-prefix enumeration — a deterministic "list every key under
  ...
  Projalpha 2026-07-24. **Justified today, independent of any multi-session
...
- **DOCKER-1** Verify the full-stack compose path (build, health,
  ...
  fleet (surveyed 2026-07-21 — Linux spokes have no Docker; hosta's

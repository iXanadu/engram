...
| **Project on a box** | **`projgamma@hosta`, `admin@hostb`** | **that project's session(s) on that host** |
| Seat (precise identity) | `projgamma-audit`, `foo-grok` | one specific session |
| Machine | `machine:hosta` | admin/maintenance sessions on that host |
...
**`<project>@<host>` is how you name "the maintenance session on hostb"
without knowing its seat.** `admin@hostb` and `admin@hosta` name different
...
can share its parent project's memory (`project = projalpha`) while needing
its own convening address. Declare it in the folder's cfg —
`groups = projalpha-app` (comma list, bare names, no `#`) — and **every**
...
in one move; `projalpha-app` did exactly this.)
...
```bash
# hub-managed rooms (Projalpha hub, port 8765; read token per box)

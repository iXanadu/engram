...
1. **A launcher can only dedupe what it launched.** A hand-started terminal
   session is invisible to Projalpha, so AB's `-2` logic cannot see it and both
...
- **A role is assigned late, to a chosen seat.** The owner picks specific seats
  ...
  The binding lives in the **huddle/orchestration layer (Projalpha)**, which
...
| **Projalpha** | *nothing mandatory* for addressing. Keeps injecting `SESSION_KEY`/`PROVIDER`/`CHANNELS`. Its `INBOX_IDENTITY` becomes a **preference** the server grants or supersedes. One UI change: **show the ordinal** (`-2`) in the session picker and read the granted seat via `/session/seats?session_key=…` instead of recomputing it |
...
`engram` on hosta and another on hostc cannot both hold `engram-claude`; the
...
2. **Broadcast stampede.** All N sessions listen on `<project>`, so one
   ...
   **orchestration** problem (Projalpha's domain per
...
`engram-claude@hosta` it is unable to recover either the project group address
(`engram`) or channel subscriptions (`#devagents`). Live proof, same session,
same call sequence:

```
memory_inbox → ['engram-claude', 'engram', 'machine:hosta', 'engram-claude@hosta', '#devagents']
memory_send  → ['engram-claude',            'machine:hosta', 'engram-claude@hosta']
```
...
address. Same class as the hardcoded `provider` that misled projalpha-app into

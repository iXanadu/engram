...
projalpha-grok-4. Cycle: v1 attacked (three kills, all accepted), v2
...
Immortal: the project channel (`projdelta`), declared sub-team groups
(`projbeta-app`), **lanes** (`projdelta-grok`), machines, humans, `#channels`.
Mortal: a session's occupant identity (`projalpha-grok-4`).
...
never-read at `projdelta-claude` vs 1,981 read at the same address — the lane
worked, the mortality failed), R8 name-locks (`projalpha-grok-2/-3/-6`),
...
- **One-time corpse drain precedes reservation** per project: existing
  ...
  run on `projdelta-claude`, marker `dead-address-drain`) and released.
...
- **[v3] Occupants are NEVER the bare lane string.** After reservation, the
  first grok on a project is not `projalpha-grok` — that string is the
...
projbeta-app's "nobody declared it"). Repo-declared groups (`groups=`) and
...
absence produced `projalpha-grok-2…-9` in one evening. Lanes do not replace
...
- The one-time corpse drains per project (projdelta-claude done as
  prototype; meidura-claude and the projalpha family pending).

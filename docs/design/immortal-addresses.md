...
owner. Reviewer requested: projalpha-grok (consumer-side half is theirs).
...
(`projdelta`), a declared sub-team group (`projbeta-app`), a **lane**
(`projdelta-grok`), a machine (`machine:hosta`), a human (`ixanadu`), a
...
- 133 open never-read messages at `projdelta-claude` — all sent in windows
...
- R8 (correct rule: a stranger never inherits a dead session's mail) turns
  ...
  R8 itself guarantees can never exist. `projalpha-grok-2/-3/-6` are locked
  today; the stale sweep is powerless (it only drains *read* mail, by design).
- Ordinal creep (`projalpha-grok-4`, `engram-claude-3`) is the visible
  symptom: respawns of the same *lane* treated as strangers competing for a
  mortal name.
- 2026-08-14 morning: mail to `projbeta-app@hosta` reached nobody because
...
lane (`projdelta-grok`) as an immortal group every successive grok session on
projdelta listens on. GROUP-1 already implements the mechanism. Lane channels
...
## Consumer-side half (Projalpha's, needs their review)
...
1. Lane channels = GROUP-1 groups, possibly auto-declared as
   ...
   listeners, explicit lanes risk the projbeta-app "nobody declared it"
...
5. One-time historical drain of existing dead-seat piles (manual prototype
   already run on `projdelta-claude`, marker `dead-address-drain`).

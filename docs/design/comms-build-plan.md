...
Participants: **engram** (this repo), **Projalpha** (their tracker), and
**projalpha-app** (becomes its own project per O2; split timing is an owner
...
- **T4 — RELAY-1 reproduced on the test itself.** The DM was described as
  "from Projalpha" and arrived stamped `✓ VERIFIED OWNER` — the relay path
...
| Item | Owner | Notes |
|---|---|---|
| projalpha-app project split (O2 corollary) | owner names timing; AB+app execute; engram advises on memory-row disposition | cfg + launcher injection + row disposition move together. |

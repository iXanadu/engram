...
dispatched to projalpha-claude under the Band-E provision; shape to the
auditor before build) · Steps 10–11 (Band D — reader-gated: one stale
hostb bridge + the owner's two Desktop bridge processes; re-census before
...
| **projalpha-claude** | Orchestration builder. Builds the AB column: relay room-only, launcher read-back, decision-object surface, status digest, transcript retention/browse, credential separation. Owns AB's tracker mapping of these items. |
| **projalpha-grok** | **Adversarial auditor.** Reviews each phase's design before its flip, holds the pre-registered verification criteria, and vetoes a flip whose evidence is short. (Holds the HUDDLE-SPEAK-1 pin already.) Supervises huddles when convened, per the pattern that worked. |
| **projalpha-app** | App-surface builder. Renders what AB's column defines (decision objects, digest, lane-keyed DM view) on the app; executes its side of the project split when the owner names it. |
...
1. **Bridge rollout sweep** — engram-claude. Update cc-memory installs on
   ...
   *(Scope confirmed by PM at kickoff, on projalpha-claude's question.)*
...
3. ∥ **Credential separation** (AB §7, HUDDLE-SPEAK-1) — projalpha-claude.
   ...
   probed live.
4. ∥ **Transcript retention confirm + browse** (AB §6) — projalpha-claude
   with projalpha-app for the surface. Confirmation ("stopped deleting")
...
9. **Allocator** (engram §2) — engram-claude, audited by projalpha-grok
   ...
   (AB §2) — projalpha-claude: never declare ordinals, read the grant from
...
10. **The flip, in four sub-steps, each announced in-thread first**
    (engram §1 + AB §1) — engram-claude + projalpha-claude, verification
    criteria held by projalpha-grok:
    ...
    **Consumer acceptance asks (projalpha-claude, accepted at kickoff, part
    ...
    **Auditor criteria (projalpha-grok, drafted at kickoff, pre-registered
...
12. **Handled-vs-read discriminator** — engram-claude designs,
    projalpha-grok adversarial review BEFORE build (this is the open hard
...
14. ∥ **Sweep tuning** (engram §4) — engram-claude. Epoch expiry for deep
    ...
    survive to climb.
15. ∥ **Decision-object surface** (AB §3) — projalpha-claude +
    projalpha-app. Asks-to-owner become stateful objects
...
16. ∥ **Status digest** (AB §4) — projalpha-claude + projalpha-app. Agent
...
17. **projalpha-app project split** (O2 corollary) — owner names timing;
    projalpha-claude + projalpha-app execute; engram-claude advises on
...
20. **Lane-keyed DM chat view** (AB §5) — projalpha-claude + projalpha-app.
...
2. projalpha-claude: map the AB column onto their tracker; flag conflicts.

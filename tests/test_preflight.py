...
def test_host_allowed_exact_and_wildcard():
    pats = ["localhost", "127.0.0.1", "hosta", "*.tailnet-demo.ts.net"]
    assert pf._host_allowed("hosta", pats)
    assert pf._host_allowed("hosta:8920", pats)            # port stripped
    assert pf._host_allowed("Hosta", pats)                 # case-insensitive

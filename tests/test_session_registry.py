...
async def test_a_displaced_process_can_never_reclaim_its_seat(client, db_pool):
    ...
    Projalpha's grok path kills a tmux session and starts its replacement
...
async def test_launcher_can_read_back_the_granted_seat(client, db_pool):
    ...
    whenever an ordinal was granted (Projalpha's provider-badge map keys on
    exactly such a reconstruction, and an unbadged row reads as a broken
    client). Requested by projalpha 2026-07-24, inside the deploy window.
    """
    await _clear(db_pool)
...
async def test_a_runtime_seat_moves_the_registration(client, db_pool):
    """ID-2, the fix Projalpha asked for: REGISTER the runtime seat.
...
async def test_seats_serve_watcher_beat_in_the_rosters_vocabulary(
    ...
    """SEATS-1 (requested by Projalpha 2026-07-28): a picker for sessions on
...
def _cert(client, **kw):
    body = {"session_key": "dead-1", "seat": f"{PROJ}-claude",
            ...
            "host": "hosta", "died_at": "2026-08-14T12:00:00Z",
            "cause": "stop"}
    body.update(kw)
...
async def test_successor_inherits_cursor_on_empty_lane(client, db_pool):
    ...
    r = await _claim(client, "new-1", host="hosta")
    seat = r.json()["seat"]
    rb = await _read_by(db_pool, "inbox/p3")
    assert f"{seat}@hosta" in rb, "successor inherited the lane cursor"

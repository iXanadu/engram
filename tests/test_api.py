...
async def test_concurrent_create_only_writes_only_one_wins(client):
    ...
    Reported by Projalpha 2026-07-26 after it destroyed real content on a
...
async def test_a_misspelled_if_match_fails_closed(client):
    ...
    — without needing to enumerate them. Traced by projalpha 2026-07-24;
...
async def test_mail_records_the_machine_it_was_sent_from(client, db_pool):
    ...
    Found closing MSG-6: a send from Hostb through its real bridge client
    ...
    r = await client.post(
        ...
        headers={"X-Engram-Machine": "hostb"},
    )
    assert r.status_code == 200
    msg_id = r.json()["id"]
...
async def test_lane5_from_lane_roundtrips(client):
    ...
    r = await client.post("/memory/send", json={
        ...
        "from_": "lane5probe-claude-2@hosta",

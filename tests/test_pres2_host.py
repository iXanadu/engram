...
async def test_host_served_on_roster(services):
    try:
        await presence_update(
            ...
            provider="claude", session_nonce="n1", host="hostb",
        )
        e = await _entry("pres2-solo")
        assert e["host"] == "hostb"
        assert e["hosts_seen"] == ["hostb"]
    finally:
        await _cleanup()
...
async def test_shared_role_multi_box_hosts_seen(services):
    ...
    try:
        await presence_update(
            ...
            provider="claude", session_nonce="mac-1", host="hosta",
        )
        await presence_update(
            identity="admin", project=PROJECT, state="running",
            provider="grok", session_nonce="web-1", host="hostb",
        )
        e = await _entry("admin")
        assert e["host"] == "hostb"  # last beater — documented, not trusted alone
        assert e["hosts_seen"] == ["hosta", "hostb"]

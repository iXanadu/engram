...
async def test_send_and_list_inbox(client, db_pool):
    ...
    resp = await client.post("/memory/inbox", json={
        "listen_set": ["engram", "machine:hosta"],
        "reader_identity": "engram@hosta",
        "unread_only": True,
    })
...
async def test_ack_marks_read_per_reader(client, db_pool):
    ...
    assert len(msgs) == 1
    assert msgs[0]["read_by"] == ["engram@hosta"]
...
async def test_ack_idempotent(client, db_pool):
    ...
    msgs = resp.json()["messages"]
    assert msgs[0]["read_by"] == ["engram@hosta"]  # only once
    await _cleanup_inbox(db_pool)
...
async def test_archive_hides_from_all_readers(client, db_pool):
    ...
    for reader in ("engram@hosta", "engram@macbook"):
...
async def test_machine_addressing(client, db_pool):
    ...
    # Admin Claude on hosta listens only on machine:hosta
    r = await client.post("/memory/inbox", json={
        "listen_set": ["machine:hosta"],
        "reader_identity": "machine:hosta",
        "unread_only": True,
    })
    msgs = r.json()["messages"]
    assert len(msgs) == 1
    assert msgs[0]["to"] == "machine:hosta"
...
async def test_banner_on_set(client, db_pool):
    ...
    resp = await client.post("/memory/set", json={
        ...
        "scope": "machine",
        "user_id": "hosta",
        "listen_set": ["engram", "machine:hosta", "engram@hosta"],
        "reader_identity": "engram@hosta",
    })
    assert resp.status_code == 200, resp.text
    data = resp.json()
...
async def test_memory_set_records_project_and_cwd_metadata(client, db_pool):
    ...
    resp = await client.post(
        ...
        headers={
            ...
            "X-Engram-Cwd": "/Users/dev/projects/engram",
            "X-Engram-Machine": "hosta",
        },
    )
    assert resp.status_code == 200, resp.text
    ...
    assert md.get("cwd") == "/Users/dev/projects/engram"
    assert md.get("machine") == "hosta"
...
async def test_case_insensitive_addressing(client, db_pool):
    ...
    resp = await client.post("/memory/inbox", json={
        "listen_set": ["projgamma", "machine:hosta"],
        "reader_identity": "projgamma@hosta",
        "unread_only": True,
    })
    msgs = resp.json()["messages"]
    assert len(msgs) == 1
...
async def test_guidance_present_on_inbox_responses(client, db_pool):
    ...
    resp = await client.post(
        ...
        json={"reader_identity": "engram@hosta"},
    )
    ack_data = resp.json()
    ...
    resp = await client.post("/memory/inbox", json={
        ...
        "reader_identity": "nonexistent@hosta",
        "unread_only": True,
    })
...
async def test_resolve_drains_from_default_view(client, db_pool):
    ...
    await _cleanup_inbox(db_pool)
    resp = await client.post("/memory/send", json={
        "to": "engram", "body": "please look", "from_": "admin@hosta",
    })
    msg_id = resp.json()["id"]
    ...
    assert msgs[0]["resolved_by"] == "engram@hosta"
...
async def test_supersede_marks_prior_message(client, db_pool):
    ...
    r2 = await client.post("/memory/send", json={
        ...
        "from_": "admin@hosta", "supersedes": old_id,
    })
    new_id = r2.json()["id"]
...
async def test_resolved_not_counted_in_banner(client, db_pool):
    ...
    await _cleanup_inbox(db_pool)
    resp = await client.post("/memory/send", json={
        "to": "engram", "body": "handle me", "from_": "admin@hosta",
    })
    msg_id = resp.json()["id"]
...
async def test_staleness_annotation(client, db_pool):
    ...
    await _cleanup_inbox(db_pool)
    resp = await client.post("/memory/send", json={
        "to": "engram", "body": "old coordination", "from_": "admin@hosta",
    })
    msg_id = resp.json()["id"]
...
async def test_digest_in_list_guidance(client, db_pool):
    ...
    keep = await client.post("/memory/send", json={
        "to": "engram", "body": "open one", "from_": "admin@hosta",
    })
    done = await client.post("/memory/send", json={
        "to": "engram", "body": "done one", "from_": "admin@hosta",
    })
...
async def test_backcompat_missing_status_is_open(client, db_pool):
    ...
    await _cleanup_inbox(db_pool)
    resp = await client.post("/memory/send", json={
        "to": "engram", "body": "legacy", "from_": "admin@hosta",
    })
    msg_id = resp.json()["id"]
...
async def test_sender_loose_form_is_used_not_fully_qualified(client, db_pool):
    ...
    addressed as 'owner1' but labels its mail 'owner1@hosta'; storing the
    qualified form would send replies to an address nobody listens on."""
    await _cleanup_inbox(db_pool)
    r = await client.post("/memory/send", json={
        "to": ["alpha", "beta"], "from_": "owner1@hosta", "body": "x",
    })
    assert r.status_code == 200
    resp = await client.post("/memory/inbox", json={
        "listen_set": ["alpha"], "reader_identity": "alpha@m", "unread_only": True,
    })
    assert "owner1@hosta" not in resp.json()["messages"][0]["participants"]
    await _cleanup_inbox(db_pool)
...
async def test_single_recipient_send_is_unchanged(client, db_pool):
    ...
    await _cleanup_inbox(db_pool)
    r = await client.post("/memory/send", json={
        "to": "alpha", "from_": "owner1@hosta", "body": "just you",
    })
    assert r.status_code == 200
    resp = await client.post("/memory/inbox", json={
        "listen_set": ["alpha"], "reader_identity": "alpha@m", "unread_only": True,
...
async def test_message_records_what_produced_it(client, db_pool):
    ...
    assert resp.status_code == 200, resp.text
    resp = await client.post("/memory/inbox", json={
        "listen_set": ["engram"], "reader_identity": "engram@hosta",
        "unread_only": True,
    })
...
def test_empty_inbox_guidance_points_at_the_transcript():
    ...
    text = inbox_list_guidance("engram-claude-2@hosta", ["engram"], msg_count=0)
...
def test_transcript_hint_only_on_the_empty_branch():
    ...
    populated = inbox_list_guidance("engram-claude-2@hosta", ["engram"], msg_count=3)

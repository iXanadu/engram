...
def test_default_identity_is_project_derived(monkeypatch):
    ...
    assert listen_set == ["projbeta", "projbeta-claude",
        "projbeta-claude@hosta", "machine:hosta", "projbeta@hosta"]
...
def test_override_gives_distinct_identity_but_keeps_project_group(monkeypatch):
    ...
    # projbeta session on hosta" without knowing its seat.
    assert listen_set == [
        "projbeta-app",
        "projbeta-claude",
        "projbeta-claude@hosta",
...
def test_seated_session_still_answers_to_project_at_host(monkeypatch):
    ...
    The operator addresses maintenance sessions as `admin@hostb` and
    `admin@hosta` — one per box, distinct, and knowable without asking what
    ...
    """
    _host(monkeypatch)
    monkeypatch.setenv(identity.INBOX_IDENTITY_ENV, "maintenance-claude-hostb")
    ...
    assert reader == "maintenance-claude-hostb@hosta"
    assert "maintenance-claude-hostb" in listen_set
    ...
    # cannot distinguish the hosta session from the hostb one. That
...
def test_two_boxes_get_distinct_project_at_host_addresses(monkeypatch):
    """`admin@hosta` and `admin@hostb` must name DIFFERENT sessions.
    ...
    monkeypatch.setattr(identity, "hostname", lambda: "hosta")
    _, on_hosta = compute_identity("/whatever")
    monkeypatch.setattr(identity, "hostname", lambda: "hostb")
    _, on_hostb = compute_identity("/whatever")

    assert "admin@hosta" in on_hosta
    assert "admin@hostb" in on_hostb
    # Neither box answers to the other's address — that is what makes the
    # convention addressable rather than ambiguous.
    assert "admin@hostb" not in on_hosta
    assert "admin@hosta" not in on_hostb
...
def test_omitted_project_dir_recalls_the_pinned_identity(monkeypatch):
    ...
    Reproduces projbeta/projalpha/admin@hosta (2026-07-18): read as the
    ...
    read_reader, _ = compute_identity("/Users/dev/projects/ProjBeta")
    assert read_reader == "projbeta@hosta"

    # Write path omits project_dir → recalls the pin instead of going admin.
    write_reader, write_set = compute_identity(None)
    assert write_reader == "projbeta@hosta"
...
def test_startup_cwd_anchors_identity_when_project_dir_omitted(monkeypatch):
    ...
    monkeypatch.setattr(identity, "_STARTUP_CWD", "/Users/dev/projects/ProjBeta")
    monkeypatch.setattr(
        identity,
        "derive_project_name",
        lambda project_dir: "projbeta"
        if project_dir == "/Users/dev/projects/ProjBeta"
...
def test_cross_project_call_does_not_move_identity(monkeypatch):
    ...
    monkeypatch.delenv(identity.INBOX_IDENTITY_ENV, raising=False)
    monkeypatch.setattr(identity, "resolve_inbox_identity", lambda _d: None)
    monkeypatch.setattr(identity, "_STARTUP_CWD", "/Users/dev/projects/Alpha")
    monkeypatch.setattr(
        identity,
        "derive_project_name",
    ...
    reader, _ = compute_identity("/Users/dev/projects/Beta")
    assert reader == "alpha@hosta"
...
def test_one_cross_project_call_does_not_cost_a_session_its_group(monkeypatch):
    ...
    `admin@hosta` from that point on, permanently.

    The failure was silent in the worst way: the watcher is anchored by a flag,
    so it never moved and still WOKE the session on mail to `admin@hosta` —
    ...
    """
    _host(monkeypatch)
    monkeypatch.setenv(identity.INBOX_IDENTITY_ENV, "maintenance-claude-hosta")
    monkeypatch.setattr(identity, "_STARTUP_CWD", "/Users/dev/maintenance")
    monkeypatch.setattr(
        identity,
        "derive_project_name",
    ...
    _, at_start = compute_identity("/Users/dev/maintenance")
    assert "admin" in at_start and "admin@hosta" in at_start

    # One cross-project read — legitimate, and must not touch identity.
    compute_identity("/Users/dev/projects/engram")

    # ...and the next ordinary call, argument omitted.
    _, after = compute_identity(None)
    assert "admin@hosta" in after, "lost its host-qualified address"
    assert "admin" in after, "lost its own project group — broadcasts stop landing"
    assert "engram@hosta" not in after, "adopted another project's address"
...
def test_watcher_anchor_wins_over_its_launching_shell_cwd(monkeypatch):
    ...
    monkeypatch.delenv(identity.INBOX_IDENTITY_ENV, raising=False)
    monkeypatch.setattr(identity, "resolve_inbox_identity", lambda _d: None)
    monkeypatch.setattr(identity, "_STARTUP_CWD", "/Users/dev")  # a bare shell
    ...
    identity.set_identity_anchor("/Users/dev/projects/Beta")
    reader, _ = compute_identity("/Users/dev/projects/Beta")
    assert reader == "beta@hosta"
...
def test_explicit_project_dir_repins_last_wins(monkeypatch):
    ...
    first, _ = compute_identity("/Users/dev/projects/Alpha")
    assert first == "alpha@hosta"
    second, _ = compute_identity("/Users/dev/projects/Beta")
    assert second == "beta@hosta"
    # Omitting now recalls the most recent explicit pin (beta), not alpha.
    recalled, _ = compute_identity("")
    assert recalled == "beta@hosta"
...
def test_relative_project_dir_does_not_pin(monkeypatch):
    ...
    monkeypatch.setattr(
        identity,
        "derive_project_name",
        lambda project_dir: "engram" if project_dir == "/Users/dev/projects/engram"
        else identity.ADMIN_NAME,
    )
    compute_identity("/Users/dev/projects/engram")  # pin engram
    reader, _ = compute_identity("relative/path")  # not absolute → recalls pin
    assert reader == "engram@hosta"
...
class TestRuntimeSeat:
    ...
    def test_runtime_seat_becomes_the_identity(self, monkeypatch):
        ...
        assert reader == "meidura-audit@hosta"
        assert listen == [
            "meidura-audit",
            "meidura-claude",
            "meidura-claude@hosta",
            "meidura",
            "meidura@hosta",
            "machine:hosta",
            "meidura-audit@hosta",
    ...
    def test_runtime_outranks_launch_env(self, monkeypatch):
        ...
        reader, _ = compute_identity("/whatever")
        assert reader == "meidura-remediate@hosta"
    ...
    def test_unset_seat_changes_nothing(self, monkeypatch):
        ...
        monkeypatch.setattr(identity, "resolve_inbox_identity", lambda _d: None)
        reader, listen = compute_identity("/whatever")
        assert reader == "meidura@hosta"
        assert listen == ["meidura", "meidura-claude", "meidura-claude@hosta",
            "machine:hosta", "meidura@hosta"]
...
class TestSeatFile:
    ...
    def test_watcher_sees_a_reseat_without_restarting(self, monkeypatch, tmp_path):
        ...
        assert after == "meidura-audit@hosta"
...
class TestAutoSessionKey:
    ...
    def test_start_time_defeats_pid_reuse(self):
        ...
        dead session's seat."""
        a = identity.auto_session_key_for(4813, "Fri Jul 24 10:29:35 2026", "hosta")
        b = identity.auto_session_key_for(4813, "Fri Jul 24 18:02:11 2026", "hosta")
        assert a != b
    ...
    def test_discovery_stops_at_the_nearest_ancestor(self, monkeypatch, tmp_path):
        ...
        near = identity.seat_file_path(
            identity.auto_session_key_for(20, "start-b", "hosta")
        )
        far = identity.seat_file_path(
            identity.auto_session_key_for(30, "start-c", "hosta")
        )
...
def test_env_override_of_a_declared_identity_is_announced(monkeypatch, tmp_path):
    ...
    wrong was the silence: a repo declared `projbeta-server`, ran as
    `projbeta-grok`, and nothing anywhere reported the divergence. Mail to the
...
class TestFolderGroups:
    ...
    project's memory (project=projalpha) and declared inbox_identity=
    projalpha-app as the team address — which launcher-injected seats then
    SHADOWED, so with three app sessions live, mail to 'projalpha-app'
    ...
    def test_seated_session_still_hears_the_team_address(
        ...
        proj = tmp_path / "Projalpha-app"
        proj.mkdir()
        (proj / ".engram.cfg").write_text(
            "project = projalpha\ngroups = projalpha-app\n"
        )
        monkeypatch.setenv(identity.INBOX_IDENTITY_ENV, "projalpha-app-grok")
        monkeypatch.setattr(identity, "hostname", lambda: "hosta")
        prev = identity._IDENTITY_ANCHOR
        try:
            identity._IDENTITY_ANCHOR = str(proj)
            reader, listen = identity.compute_identity(str(proj))
            assert reader == "projalpha-app-grok@hosta"
            # seat, project group, AND the team group — all present
            assert "projalpha-app-grok" in listen
            assert "projalpha" in listen
            assert "projalpha-app" in listen, (
                "the folder's team address must be heard whatever the seat"
            )
            assert "projalpha-app@hosta" in listen
        finally:
...
class TestInv1ProjectChannelAlways:
    ...
    def _assert_project_channel(self, listen):
        ...
        assert "proj@hosta" in listen, f"project@host missing: {listen}"
...
class TestLane2ListenSet:
    ...
    def test_seated_ordinal_occupant_hears_its_lane(self, monkeypatch):
        ...
        projdelta-claude while the session ran as an ordinal."""
        self._base(monkeypatch)
        monkeypatch.setenv(identity.INBOX_IDENTITY_ENV, "proj-claude-4")
        _, listen = compute_identity("/whatever")
        assert "proj-claude" in listen
        assert "proj-claude@hosta" in listen
        # nothing displaced (INV-1 + occupant)
        assert "proj-claude-4" in listen
        assert "proj" in listen and "proj@hosta" in listen
    ...
    def test_provider_env_selects_the_lane(self, monkeypatch):
        ...
        monkeypatch.setenv(identity.INBOX_IDENTITY_ENV, "proj-grok-2")
        _, listen = compute_identity("/whatever")
        assert "proj-grok" in listen and "proj-grok@hosta" in listen
    ...
    def test_bare_unseated_session_hears_its_lane(self, monkeypatch):
        self._base(monkeypatch)
        _, listen = compute_identity("/whatever")
        assert "proj-claude" in listen and "proj-claude@hosta" in listen
    ...
    def test_declared_group_equal_to_lane_not_duplicated(self, monkeypatch):
        ...
        _, listen = compute_identity("/whatever")
        assert listen.count("proj-claude") == 1
        assert listen.count("proj-claude@hosta") == 1
...
class TestTzStableProcessWatch:
    """hostc 2026-08-15: `ps lstart` renders in the CURRENT tz/locale, so a
...
def test_seat_claim_project_is_anchored_not_moved_by_a_cross_project_memory_call(monkeypatch):
    ...
    became `projepsilon-claude-2` mid-huddle, its huddle reply was refused
    ...
    _host(monkeypatch)
    monkeypatch.setattr(identity, "_IDENTITY_ANCHOR", None)
    monkeypatch.setattr(identity, "_STARTUP_CWD", "/Users/dev/projects/engram")
    monkeypatch.setattr(identity, "_SESSION_PROJECT_DIR", None)
    names = lambda d: "projepsilon" if (d or "").endswith("Projepsilon") else "engram"
    ...
    server.remember_project_dir("/Users/dev/projects/Projepsilon")
    # Memory scoping followed the call (its job)...
    assert server.derive_project_name(server.remember_project_dir(None)) == "projepsilon"

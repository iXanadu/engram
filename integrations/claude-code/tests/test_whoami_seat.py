...
async def test_whoami_prints_granted_seat_and_listen_set(monkeypatch):
    ...
    def _ident(project_dir):
        ...
        return ("proj-grok-2@hosta",
                ["proj-grok-2", "proj-grok", "proj", "machine:hosta"])
    ...
    assert "Inbox identity (your granted seat): proj-grok-2@hosta" in out
    assert "Listening on: ['proj-grok-2', 'proj-grok', 'proj', 'machine:hosta']" in out

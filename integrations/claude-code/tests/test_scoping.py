...
def test_resolve_inbox_identity_from_cfg(tmp_path):
    (tmp_path / ".engram.cfg").write_text(
        "project = projbeta\ninbox_identity = projbeta-server\n"
    )
    # project and inbox identity come from the SAME file but are independent
    assert resolve_project_name(str(tmp_path)) == "projbeta"
    assert resolve_inbox_identity(str(tmp_path)) == "projbeta-server"


def test_resolve_inbox_identity_absent_is_none(tmp_path):
    (tmp_path / ".engram.cfg").write_text("project = projbeta\n")
    assert resolve_project_name(str(tmp_path)) == "projbeta"
...
def test_resolve_inbox_identity_walks_up(tmp_path):
    (tmp_path / ".engram.cfg").write_text(
        "project = projbeta\ninbox_identity = projbeta-app\n"
    )
    sub = tmp_path / "ios" / "src"
    sub.mkdir(parents=True)
    assert resolve_inbox_identity(str(sub)) == "projbeta-app"
...
def test_resolve_inbox_groups_parses_and_normalizes(tmp_path):
    (tmp_path / ".engram.cfg").write_text(
        "project = projalpha\ngroups = Projalpha-App,  extra-team ,\n"
    )
    assert resolve_inbox_groups(str(tmp_path)) == ["projalpha-app", "extra-team"]


def test_resolve_inbox_groups_absent_is_empty(tmp_path):
    (tmp_path / ".engram.cfg").write_text("project = projalpha\n")
...
def test_resolve_inbox_groups_rejects_channel_sigil(tmp_path):
    ...
    (tmp_path / ".engram.cfg").write_text(
        "project = projalpha\ngroups = #devagents, projalpha-app\n"
    )
    assert resolve_inbox_groups(str(tmp_path)) == ["projalpha-app"]

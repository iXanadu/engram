...
async def test_memory_store_admin_sends_admin_provenance(respx_mock):
    ...
    assert req.headers["x-engram-cwd"] == "/Users/dev"
    import json as _json
    body = _json.loads(req.content)
    assert body["reader_identity"] == "admin@hosta"
    assert body["listen_set"] == ["admin", "admin@fleet", "machine:hosta", "admin@hosta"]
        ...
        return_value=httpx.Response(
            ...
            json={
                ...
                "inbox_banner": {
                    "unread_count": 2,
                    "preview": [
                        "admin@hosta → engram: restart needed",
            ...
            json={
                ...
                "keys": [
                    {"namespace": "fleet", "key": f"k/{i}", "scope": "machine",
                     "user_id": "hosta", "value_chars": 1, "status": None}
                    for i in range(200)
                ],
            },
        )

...
async def test_poll_skips_self_echo(respx_mock):
    # reader is projbeta@hosta; its own sends carry from=projbeta@hosta
    # (full form) — and its loose name "projbeta" is equally self. Both must be
    ...
    respx_mock.post("/memory/inbox").mock(
        return_value=httpx.Response(
            ...
            json={
                "status": "ok",
                "messages": [
                    _msg(1, **{"from": "projbeta@hosta"}),  # own outbound, full form
                    _msg(2, **{"from": "projbeta"}),          # own outbound, loose form
                    _msg(3, **{"from": "engram@hosta"}),     # genuine peer
                ],
            },
        )
    )
    c = MemoryClient("http://localhost:8920", "")
    seen: set = set()
    fresh = await _poll(c, ["projbeta"], "projbeta@hosta", seen)
...
async def test_poll_sibling_survives_under_distinct_identities(respx_mock):
    # With a per-session identity (projbeta-app), the sibling session
    # (projbeta-server) is NOT self — its mail must wake us; only our own
    # outbound is dropped.
    respx_mock.post("/memory/inbox").mock(
        return_value=httpx.Response(
            200,
            json={
                "status": "ok",
                "messages": [
                    _msg(1, **{"from": "projbeta-app@hosta"}),     # own echo → drop
                    _msg(2, **{"from": "projbeta-server@hosta"}),  # sibling → keep
                ],
            },
        )
    )
    c = MemoryClient("http://localhost:8920", "")
    fresh = await _poll(c, ["projbeta-app", "projbeta"], "projbeta-app@hosta", set())
...
def test_digest_is_one_line_counts_only_immortal(capsys):
    ...
    backlog = [
        ...
        _msg(7, to="machine:hosta"),        # machine → excluded
    ]
...
def test_105_messages_are_one_line_with_a_count(capsys):
    backlog = [_msg(i, to="projdelta-claude") for i in range(105)]
    assert _emit_backlog_digest(backlog, occupant="projdelta-claude-2",
                                project="projdelta") == 105
    lines = capsys.readouterr().out.strip().splitlines()
    assert len(lines) == 1
    obj = json.loads(lines[0])
    assert obj["addresses"]["projdelta-claude"]["unread_count"] == 105
...
def test_plaintext_warning_for_remote_http(capsys):
    ...
    _warn_plaintext_url("http://hosta:8920")

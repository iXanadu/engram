...
# wrong sender. Reported 2026-07-18 by projbeta / projalpha@hosta /
# admin@hosta — the read/write identity divergence bug.
#
...
def identity_anchor_dir() -> str | None:
    ...
    addresses for the rest of its life:
        ...
        start, pinned to ~/maintenance   -> ['admin', 'admin@hosta', ...]
        one call scoped elsewhere        -> ['engram', 'engram@hosta', ...]
        every call after, arg omitted    -> ['engram', 'engram@hosta', ...]
...
def resolve_session_identity(project_dir: str | None) -> str | None:
    ...
    if env:
        ...
        # nobody. Measured 2026-08-02: a project declared `projbeta-server`,
        # ran as `projbeta-grok`, and the dead address went unnoticed long
...
def _ps_stable_env() -> dict:
    ...
    recycled pid. Found live on hostc 2026-08-15 during the fleet's UTC →
...
def resolve_session_key() -> str | None:
    ...
    A launcher-injected key is preferred — Projalpha's tmux name is unique per
...
def resolve_provider() -> str:
    ...
    misled by it (projalpha-app, 2026-07-23) and reported the constant to the
...
def compute_identity(project_dir: str | None) -> tuple[str, list[str]]:
    ...
    if override and override != project:
        ...
        # operator addresses by: `admin@hostb` and `admin@hosta` name the

...
def resolve_inbox_identity(project_dir: str | None) -> str | None:
    ...
    (e.g. ``inbox_identity = projbeta-server`` vs ``projbeta-app``). It is the
...
def resolve_inbox_groups(project_dir: str | None) -> list[str]:
    ...
    parent project's memory brain (``project = projalpha``) while needing
    its own convening address (``projalpha-app``). ``inbox_identity`` was
    ...
    ``projalpha-app`` reached whichever single session happened to hold

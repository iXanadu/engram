...
{
  "status": "ok",
  "read":  ["claude-code", "projbeta", "ha", ...],
  "write": ["claude-code", "projbeta", "ha", ...]
}
```
...
project       = projbeta          # shared memory bucket — same for both sessions
inbox_identity = projbeta-server  # distinct inbox address — different per session
```

```bash
# ENGRAM_INBOX_IDENTITY env var — overrides the file when set (escape hatch)
export ENGRAM_INBOX_IDENTITY=projbeta-server
```
